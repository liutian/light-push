...
- 如果检测到超过30s没有发送消息或者接收到消息则主动进行消息同步  
- 对每一条发送出去的消息进行匹配，如果包含“在吗|有人吗|人呢”等关键字，或者连续两次以上发送的消息完全一样(只针对文字类型的消息)，启动在线状态检测 
- 如果发出去的消息没被对方接收(看1)，则启动在线状态检测  
- 消息同步时发现不一致，则切换本地推送连接方式为：polling 
- 当发生重连操作时切换推送连接方式为：polling 
``` 
  socket.on('reconnect_attempt', () => {
    socket.io.opts.transports = ['polling'];
  })
```
- 在线状态检测：本地检测通过主动访问推送服务器来检测 `socket.emit('info', data, callback)` ，对方检测通过调用应用服务器来确认(应用服务器调用推送服务器)，
   检测之前设置一个定时器，延迟5s判断最终结果。(5s之后没有得到结果也被判定为离线)如果有一方离线则提示：“网络异常”，同时上报这次离线检测结果(只有发生离线时才上报)，之后启动消息同步  
- 消息同步：查询服务器上最新的消息ID，同时和本地最新消息ID进行比较，如果不一致，则刷新当前消息列表，或者将丢失的消息追加到消息列表  
